...
export function bonifierCalculator(attack, attacker = {}, target = {}) {

    console.log('attack.type:', attack?.type)
    console.log('attacker.types:', attacker.types)
    console.log('target.types:', target.types)
    console.log('target.damageClass:', target.damageClass)
    
  const attackType = attack?.type
  const attackerTypes = attacker.types?.map((t) => t?.type?.name).filter(Boolean) ?? []
  const targetTypes = target.types?.map((t) => t?.type?.name).filter(Boolean) ?? []

  if (!attackType) return 1

  const stab = attackerTypes.includes(attackType) ? 1.5 : 1

  const effectiveness = targetTypes.reduce((multiplier, defenderType) => {
